...
function solitaireGame(oUniqueName, oWorkspace, oDeck, oWastePile, oFoundation, oTableau, oCardWord, oCardNames, oSuits, youWin, oDeal, oOptions,
    ...
    // Checa por configurações de parametros
    ...
    this.strings.prefDone = prefDone ? prefDone : 'Pronto';
    this.strings.prefCanc = prefCanc ? prefCanc : 'Cancelar';
    this.strings.prefName = prefName ? prefName : 'Opcoes';
    this.strings.imgPref = imgPref ? imgPref : 'Tamanho das cartas (pixels) ';
    this.strings.dealPref = dealPref ? dealPref : 'Distribuicao';
    this.strings.cardPref = cardPref ? cardPref : 'Tipo de carta:';
    this.strings.backPref = backPref ? backPref : 'Imagem de fundo:';
    this.strings.deal1 = deal1 ? deal1 : 'Dar 1';
    this.strings.deal3 = deal3 ? deal3 : 'Dar 3';
    ...
    for (var i = 0; i < this.cards.playingCards.length; i++) {
        ...
        this.cards.playingCards[i].representation.ondragstart = function () {
            return false;
        };
        if (window.opera) {
            ...
            this.cards.playingCards[i].representation.onblur = function () {
                this.style.border = '';
                this.style.margin = '';
            };
        }
...
solitaireGame.prototype.throwError = function (oName, oMessage, oSurpress) {
    ...
    if (!window.hideCardGameErrors && !oSurpress) {
        alert('Fatal error: ' + oName + '\n\n' + oMessage);
    }
    throw({name: oName, message: oMessage});
};

solitaireGame.prototype.setCheatMode = function (oMode) {
    this.cheatsAllowed = oMode;
};

solitaireGame.prototype.toString = function () {
    return '[object solitaireGame: ' + this.name + ']';
};
...
solitaireGame.prototype.checkForWinner = function () {
    ...
    for (var i = 2; i < 6; i++) {
        if (this.cardStacks[i].cardsInStack.length != 13) {
            return;
        }
    }
    // They won
...
playingCard.prototype.autoPositionSolitaire = function () {
    ...
    if (this.representation.style.opacity && this.representation.style.opacity != '1') {
        ...
        if (this.representation.style.setProperty) {
            // Safari 1.1
            try {
                tmpCard.representation.style.setProperty('-khtml-opacity', '1');
            } catch (e) {
            }
        }
    }
...
playingCard.prototype.dragIsStartingSolitaire = function () {
    ...
    if (this.nextOnStack()) {
        this.nextOnStack().dragIsStarting();
    }
};
...
playingCard.prototype.dragHasStoppedSolitaire = function () {
    ...
    if (this.nextOnStack()) {
        this.nextOnStack().dragHasStopped();
    }
};
...
playingCard.prototype.moveToStackIfPossible = function (oStack, oCheat) {
    ...
    if (oStack.type == 2) {
        if (this.nextOnStack()) {
        } else if ((this.number == 1) && (oStack.cardsInStack.length == 0)) {
            canMove = true;
        } else if ((this.number != 1) && oStack.cardsInStack.length && (oStack.cardsInStack[oStack.cardsInStack.length - 1].number == this.number - 1) && (oStack.cardsInStack[oStack.cardsInStack.length - 1].suit == this.suit)) {
            canMove = true;
        }
    } else if (oStack.type == 3) {
        if (oCheat && (oStack.cardsInStack[oStack.cardsInStack.length - 1] != this)) {
            canMove = true;
        }
        if (oStack.cardsInStack.length == 0) {
            ...
            if (oStack.cardsInStack[oStack.cardsInStack.length - 1].wayup && (oStack.cardsInStack[oStack.cardsInStack.length - 1].number == this.number + 1) && (oStack.cardsInStack[oStack.cardsInStack.length - 1].color != this.color)) {
                canMove = true;
            }
            oCheat = false;
        }
    ...
    if (canMove) {
        ...
        if (this.game.doingSpatNav && window.opera) {
            this.game.workspace.className += '';
        } //outline redraw fix (spatnav only)
...
solitaireGame.prototype.handleClicksOnHotspots = function (e) {
    if (!this.relatedObject.game.gameInPlay) {
        return;
    }
    if (this.downOnCard || this.moveOnCard) {
        this.clearMouseEvents();
    ...
    if (this.relatedObject.game.doingSpatNav) {
        ...
        if (this.relatedObject.game.onresetdeck && !this.relatedObject.game.onresetdeck({type: 'resetdeck', target: this.relatedObject, currentTarget: this.relatedObject, relatedTarget: this.relatedObject.game.cardStacks[1]})) {
            return;
        }
...
solitaireGame.prototype.handleDoubleClicksOnCards = function () {
    ...
    if (!this.relatedObject || !this.relatedObject.game.gameInPlay) {
        return;
    }
    if (this.downOnCard || this.moveOnCard) {
        this.clearMouseEvents();
    ...
    setTimeout(function () {
        relObj.wasDragged = false;
    }, 20); //in case browser fires dblclick before click
    if (this.relatedObject.cardStack.type == 2) {
        return;
    }
    var currentStack = this.relatedObject.cardStack;
    for (var i = 2; i < 6; i++) {
        if (this.relatedObject.moveToStackIfPossible(this.relatedObject.game.cardStacks[i])) {
            return;
        }
    }
};
...
solitaireGame.prototype.handleClicksOnCards = function (e) {
    ...
    if (!ontop && ((this.relatedObject.cardStack.type != 3) || !this.relatedObject.wayup)) {
        return;
    }
    ...
    if (this.relatedObject.game.doingSpatNav) {
        ...
        if (this.relatedObject.game.onwastepile && !this.relatedObject.game.onwastepile({type: 'wastepile', target: this.relatedObject, currentTarget: this.relatedObject, relatedTarget: this.relatedObject.game.cardStacks[1], threeCards: eachCard, dealNum: this.relatedObject.game.dealNum})) {
            return;
        }
        ...
        if (this.relatedObject.game.onflipcard && !this.relatedObject.game.onflipcard({type: 'flipcard', target: this.relatedObject, currentTarget: this.relatedObject})) {
            return;
        }
        ...
        while (tmpCard) {
            ...
            if (tmpCard.representation.style.setProperty) {
                // Safari 1.1
                try {
                    tmpCard.representation.style.setProperty('-khtml-opacity', '0.4');
                } catch (e) {
                }
            }
        ...
        if (window.opera) {
            this.relatedObject.game.workspace.className += '';
        } //outline redraw fix
    }
...
solitaireGame.prototype.handleMousedownOnCards = function (e) {
    ...
    if (solitaireGame.prototype.downOnSolitaireCard) {
        return;
    }
    if (this.relatedObject.game.doingSpatNav) {
        return;
    ...
    if (!(ontop && this.relatedObject.cardStack.type && (this.relatedObject.cardStack.type < 3)) && !(this.relatedObject.wayup && (this.relatedObject.cardStack.type == 3))) {
        return;
    }
...
solitaireGame.prototype.showPrefs = function (e) {
    ...
    cancelBut.onclick = function () {
        if (oForm.relatedGame.oncancelprefs && !oForm.relatedGame.oncancelprefs({type: 'cancelprefs', target: oForm.relatedGame, currentTarget: oForm.relatedGame})) {
            return;
        }
    ...
    submitBut.onclick = function () {
        ...
        if (oForm.relatedGame.onsaveprefs && !oForm.relatedGame.onsaveprefs({type: 'saveprefs', target: oForm.relatedGame, currentTarget: oForm.relatedGame, cardSet: secndSelect.cardSet, cardBack: secndSelect.backs[secndSelect.selectedIndex][0], dealNum: thirdSelect.selectedIndex ? 3 : 1})) {
            return;
        }
